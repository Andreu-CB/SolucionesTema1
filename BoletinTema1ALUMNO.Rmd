...
#a
help(mammals, package = MASS)
help(Animals2, package = robustbase)
...
# Hay cuatro animales que no estan en ambos conjuntos y son: "Arctic fox", "Arctic ground squirrel", "Patas monkey", "Mole rat"
```
...
# Ambas gráficas presentan el mismo comportamiento
# Se podria decir que siguen una distribución gausiana
```
...
#a
str(cabbages)
# El conjunto de datos cabbages tiene 60 registros y 4 variables, dos de tipo Factor, una de tipo numérico y una de tipo entero

#b
sum(is.na(cabbages$Cult)*1)
sum(is.na(cabbages$Date)*1)
sum(is.na(cabbages$HeadWt)*1)
sum(is.na(cabbages$VitC)*1)

#c
str(Chile)
#El conjunto de datos Chile tiene 2700 registros y 8 variables, cuatro de tipo Factor, tres de tipo entero y una de tipo numérico

sum(is.na(Chile$region)*1)
sum(is.na(Chile$population)*1)
sum(is.na(Chile$sex)*1)
sum(is.na(Chile$age)*1)
sum(is.na(Chile$education)*1)
sum(is.na(Chile$income)*1)
sum(is.na(Chile$statusquo)*1)
sum(is.na(Chile$vote)*1)
...
#Ambas la verifican ya que todos sus valores se encuentran entre las líneas discontinuas
```
...
```{r}
hist(cabbages[cabbages$Cult == 'c39',4], main = 'Frecuencia de vitamina C en la variedad c39', xlab = 'Vitamina C', ylab = 'Frecuencia')
hist(cabbages[cabbages$Cult == 'c52',4], main = 'Frecuencia de vitamina C en la variedad c52', xlab = 'Vitamina C', ylab = 'Frecuencia')
# En la variedad c39 vemos mas frecuencia entre los valores 40 y 55 y en la c52 vemos mas frecuencia entre los valores 55 y 75
...
abline(Model)
# Los datos son muy dispersos por lo que no se aprecia una relación lineal
